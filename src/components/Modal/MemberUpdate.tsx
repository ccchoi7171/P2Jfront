...
const MemberUpdateModal: React.FC<Props> = ({ user, onClose }) => {
  ...
  const [currentProfileImageURL, setCurrentProfileImageURL] = useState(user.profileImage || "");
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await axios.put("http://localhost:8080/api/members", formData, {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.profileImage) setCurrentProfileImageURL(res.data.profileImage);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-2xl p-8 w-full max-w-[520px] shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">회원 정보 수정</h2>
        ...
        <div className="space-y-5">
          <div>
            ...
            <p className="w-full px-4 py-2 border rounded-md bg-gray-50">{nickname}</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">이메일</label>
            <p className="w-full px-4 py-2 border rounded-md bg-gray-50">{email}</p>
          </div>

          {currentProfileImageURL && (
            <div>
              <p className="text-sm font-medium text-gray-700 mb-2">현재 프로필 사진</p>
              <img
